...
mod HackTemplate {
    ...
    impl HackTemplateABIImpl of HackTemplateABI<ContractState> {
        ...
        let output: PragmaPricesResponse = oracle_dispatcher
            .get_data_for_sources(DataType::SpotEntry(BTC_USD),AggregationMode::Median(()), sources.span());
    ...
    fn realized_volatility(self : @ContractState) -> (u128, u32) {
        let oracle_dispatcher = ISummaryStatsABIDispatcher { contract_address: self.summary_stats.read() };
        ...
        let (volatility, decimals) = oracle_dispatcher
            .calculate_volatility(DataType::SpotEntry(key), start.into(), end.into(), num_samples, AggregationMode::Median(()));

        let (mean, mean_decimals) = oracle_dispatcher.calculate_mean(DataType::SpotEntry(key), start.into(), end.into(), AggregationMode::Median(()));
